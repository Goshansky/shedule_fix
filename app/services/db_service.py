...
from app.models.history_has_group import HistoryHasGroup
from app.services.models_service import get_history_by_text, get_events_by_group_id, get_long_breaks_by_group_id, \
    get_short_breaks_by_group_id, get_different_buildings_by_group_id, get_events_lb_by_long_break_id, \
    get_events_sb_by_short_break_id, get_events_db_by_different_building_id, add_history, add_group, add_event, \
    add_long_break, add_short_break, add_different_building
...
async def get_schedule_2(query: str = None, db: Session = Depends(get_db)):
                ...
                "different_buildings": [
                    {
                        ...
                        "events_by_building": [
                            {list(set(event.location[event.location.find("(") + 1:-1] for event in db.events))[i]:
                                 [event for event in db.events if
                                  list(set(event.location[event.location.find("(") + 1:-1] for event in db.events))[
                                      i] in event.location]}
                            for i in range(
...
async def get_different_buildings_2(query: str = None, db: Session = Depends(get_db)):
    # Проверяем, был ли выполнен такой запрос ранее
    history = get_history_by_text(db, query)
    if history:
        # Возвращаем данные из базы данных
                    ...
                    "different_buildings": [
                        {
                            ...
                            "events_by_building": [{list(
                                set(event.location[event.location.find("(") + 1:-1] for event in db.events))[i]:
                                                   ...
                                                   for i in
                                                   range(len(list(set(event.location[event.location.find("(") + 1:-1]
...
async def get_long_breaks_2(query: str = None, db: Session = Depends(get_db)):
    # Проверяем, был ли выполнен такой запрос ранее
    history = get_history_by_text(db, query)
    if history:
        # Возвращаем данные из базы данных
...
async def get_short_breaks_different_campus_2(query: str = None, db: Session = Depends(get_db)):
    # Проверяем, был ли выполнен такой запрос ранее
    history = get_history_by_text(db, query)
    if history:
        # Возвращаем данные из базы данных
